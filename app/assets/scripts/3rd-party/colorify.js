(function(root, factory) {
  ...
  var colorify = function(args) {
    ...
    if (!_REVEAL_ON == false) {
      ...
      document
        .querySelector(_REVEAL_ON_TRIGGER)
        .addEventListener(_REVEAL_ON_EVENT, function() {
    ...
    setTimeout(function() {
      ...
      function isLoaded(el, pos) {
        ...
        if (_GRADIENT) {
          ...
          el.parentNode.style.backgroundImage =
            "linear-gradient(" + _GRADIENT_DIR + "," + gradient + ") ";
        } else {
          el.parentNode.style.backgroundColor =
            "rgb(" + rgb[0].r + "," + rgb[0].g + "," + rgb[0].b + ")";
        }
        ...
        if (!_GIVE == false) {
          ...
          if (_TARGET == "parent") {
            el.parentNode.parentNode.setAttribute(
              "style",
              _PROPERTY +
                ": rgb(" +
                rgb[0].r +
        ...
        if (
          (!_TRANSITION == false && !_DELAY == false) ||
          !_TRANSITION == false ||
          !_DELAY == false
        ) {
          if (_STEPPED == true) {
            el.setAttribute(
              "style",
              "transition: all " + _TRANSITION + "s ease " + _DELAY * pos + "s;"
            );
          } else {
            el.setAttribute(
              "style",
              "transition: all " + _TRANSITION + "s ease " + _DELAY + "s;"
            );
          }
        }
      ...
      function getAverageRGB(imgEl) {
        ...
        if (_GRADIENT) {
          try {
            var rgbStart = getAverageRGBFromZone(
              context.getImageData(0, 0, width / 4, height / 4),
              { accuracy: _ACCURACY }
            );
            var rgbEnd = getAverageRGBFromZone(
